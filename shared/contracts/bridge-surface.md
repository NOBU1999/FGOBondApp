...
> **契约版本：1**（2026-09-18 阶段 3 固化）。版本号只在本文件与 `engine-protocol.md` 里维护；
> 破坏性变更（改名 / 改参数 / 改错误文案）→ 版本 +1，并同步改所有平台实现与 `renderer/`。
...
```bash
npm run test:contracts -- --suite bridge     # 33 条用例，覆盖下列全部行为
```

- 用例文件：`tests/contracts/bridge-cases.json`（**改行为就要改这里**，别只改代码）
- 运行器：`tests/run-contracts.mjs`；换平台宿主：`--host <模块>`（导出 `createHost({dbPath})`）
- 已验证的错误文案（**以实际为准**）：
  - `账号名称不能为空` · `账号不存在` · `至少保留一个账号`
  - `账号不存在或已被删除，请重新选择账号`（写入时账号已失效）
- 错误传递方式：方法**抛异常** ⇒ 桥接层 reject（`invoke` 的 Promise 失败）；界面 catch 后展示 `err.message`。
  正常返回值**原样**透传，不做包装（`{ok:true,...}` 只是部分方法自己的返回结构，不是统一信封）。
